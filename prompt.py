...
def generate_prompt(client_query, products, clients_data):
    ...
    for product in products:
        product_list_str += f"- ID: {product['id']}, Name: {product['name']}, Description: {product['description']}, " \
                            ...
                            f"Price: {product['price']}, Times Purchased: {product['sold']}, Avg Rating: {product['avg_rating']}\n"
    ...
    for client in clients_data:
        ...
        if product:
            rated_products_str += f"  - ID: {product['id']}, Name: {product['name']}, Rating: {client['rating']}\n"
...
Below is a list of products, each with its ID, name, description, category, price, times purchased, and average rating.

Product List:
{product_list_str}

Also, here is data about clients' previous ratings:

{clients_data_str}

A customer named "{client_name}" has requested personalized suggestions based on their previous product ratings.

Your task is to:

1. **Analyze Client's Ratings**: Review the client's past ratings to understand their preferences. Focus on the products they rated highly (e.g., 4 or 5).

2. **Expert Selection**: Select the most appropriate products from the product list that match the client's preferences, based on the ones they rated highly. Suggest products with similar styles, categories, or features.
...
A customer has requested suggestions for popular products that are trending based on ratings.

Your task is to:

1. **Select Popular Products**: From the product list, select products that have the highest average ratings.
...
1. **Understand the Inquiry**: Carefully read the customer's inquiry to understand their needs and preferences.

2. **Expert Selection**: Select the most appropriate products from the product list that match the customer's needs, considering ratings and product features.
